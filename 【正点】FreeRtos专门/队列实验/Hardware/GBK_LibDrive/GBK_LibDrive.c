...
void GBK_ReadID(void)
{
	u8 Temp[24];
	u8 i;
	LCD_Clear(background_color); // 清除屏幕
	for (i = 0; i < 24; i++)
		Temp[i] = 0; // 初始化缓冲区
...
void GBK_ShowASCII(uint16_t x, uint16_t y, uint8_t *N_Word, uint8_t size, uint16_t D_Color, uint16_t B_Color, uint8_t mode)
{
	...
	for (t = 0; t < csize; t++)
	{
		...
		{
			if (temp & 0x80)
                LCD_Draw_Point_FSMC(x, y, D_Color);
			else if (mode == 0)
				LCD_Draw_Point_FSMC(x, y, B_Color); // 显示背景颜色
			temp <<= 1;
			y++;
			if ((y - y0) == size)
			{
	...
	for (t = 0; t < csize; t++)
	{
		...
		for (t1 = 0; t1 < 8; t1++)
		{
			if (temp & 0x80)
                ...
                LCD_Draw_Point_FSMC(x, y, B_Color);
			temp <<= 1;
...
void GBK_Show_StrMid(uint16_t x, uint16_t y, uint8_t size, uint8_t len, uint16_t D_Color, uint16_t B_Color, uint8_t *str)
{
	...
	strlenth *= size / 2;
	if (strlenth > len)
		GBK_Show_Str(x, y, lcd.width, lcd.height, str, size, D_Color, B_Color, 1);
	else
	{
		strlenth = (len - strlenth) / 2;
		GBK_Show_Str(strlenth + x, y, lcd.width, lcd.height, str, size, D_Color, B_Color, 1);
	}
}
