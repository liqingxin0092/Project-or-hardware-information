...
uint8_t rtc_init(void)
{
    ...
    if ((bkpflag != 0x5055) && (bkpflag != 0x5056)) /* 之前未初始化过, 重新配置 */
    {
        rtc_set_time(2, 45, 50, RTC_HOURFORMAT12_AM); /* 设置时间, 根据实际时间修改 */
        rtc_set_date(24, 11, 17, 7);                  /* 设置日期 */
    }
    return 0;
}
 ...
 *   @arg       RTC_WAKEUPCLOCK_CK_SPRE_16BITS      ((uint32_t)0x00000004) 固定1Hz
 ...
 * @param       cnt: 自动重装载值.减到0,产生中断. 实际值等于这个+1
 * @retval      无
 */
void rtc_set_wakeup(uint8_t wksel, uint16_t cnt)
{
    ...
    HAL_NVIC_SetPriority(RTC_WKUP_IRQn, 2, 0); /* 抢占优先级2, 子优先级2 */
...
{
    DMA2D_fill(450, 450, 500, 500, GREEN);
}
...
{
    LTDC_turn_area_color(1005, 586, 1023, 599, YELLOW, BLUE);
}
