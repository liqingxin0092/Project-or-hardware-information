...
void lcd_spi_init(void)
{
    ...
    // 7:4,         0,    MIDI,数据间无延时
    // 3:0,         0,    第一个延时
    ACTIVE_SPI->CFG2 = (vu32)0xA0420000;
    
    while(ACTIVE_SPI->CFG2!=(vu32)0xA0420000)  //有时候设置一遍设置不上
    {
        RCC->APB2RSTR|=1<<13;		//复位SPI4
        delay_us(2);
        RCC->APB2RSTR&=~(1<<13);	//停止复位SPI4
...
void my_spi_transmit_8bit(volatile uint8_t * src,uint16_t size)
{
    u16 fercount=size;
...
//这里只针对16bit
//使用前 加上 __SPI_16BIT();
void my_spi_transmit_16bit( uint8_t * src,uint16_t size)
{
    u16 fercount=size;
    ACTIVE_SPI->CR2=size;//配置传输长度,这里直接=size,因为其他位就是0.
    ACTIVE_SPI->CR1=1;//enable spi,等于1,原因同上
...
void LCD_DrawPoint(uint16_t x,uint16_t y,uint32_t color)
{
	...
	my_spi_transmit_16bit( (uint8_t*) &color,1);
}
