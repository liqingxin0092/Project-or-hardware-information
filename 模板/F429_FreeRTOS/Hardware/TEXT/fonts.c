...
char *const FONT_GBK_PATH[5] =
{
    ...
    "/SYSTEM/FONT/GBK32.FON",       /* GBK32的存放位置 */ //未实现占太大内存了,约3MB
};
...
static void fonts_progress_show(uint16_t x, uint16_t y, uint8_t size, uint32_t totsize, uint32_t pos, uint16_t color)
{
    ...
    if (t != prog)
    {
        LTDC_ShowString(x + 3 * size / 2, y, 320, 240,  size, "%", color,1);
        t = prog;
        if (t > 100)t = 100;
        LTDC_ShowxNum(x,  y, t, 3,  size, 1,color);
    }
}
    ...
    if (rval == 0)
    {
        switch (fx)
        {
            case 0: /* 更新 UNIGBK.BIN */
                ...
                ftinfo.ugbksize = fftemp->obj.objsize;//UNIGBK大小,前面读了,获取这个文件的大小
...
//                break;
        ...
        while (res == FR_OK)   /* 死循环执行 */
        {
            ...
            W25QXX_Write(tempbuf, offx + flashaddr, bread);  /* 从0开始写入bread个数据 */
            offx += bread;
...
uint8_t fonts_update_font(uint16_t x, uint16_t y, uint8_t size, uint8_t *src, uint16_t color)
{
    ...
    if (rval == 0)          /* 字库文件都存在. */
    {
        LTDC_ShowString(x, y, 320, 240, size, "Erasing sectors... ", color,  1);
        ...
        {
            LTDC_ShowString(x,  y, 320, 240, size, FONT_UPDATE_REMIND_TBL[i], color, 1);
            strcpy((char *)pname, (char *)src);                                     /* copy src内容到pname */
            strcat((char *)pname, (char *)FONT_GBK_PATH[i]);                        /* 追加具体文件路径 */
