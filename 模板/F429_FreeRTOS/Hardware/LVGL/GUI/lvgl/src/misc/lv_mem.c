...
{
#if LV_MEM_CUSTOM == 0 // 3 //是不是LV自带算法

#if LV_MEM_ADR == 0 // 2  //给没给出地址,这里是没给出地址

#ifdef LV_MEM_POOL_ALLOC // 1
    // 如果用LV自带算法,不给出地址,并且启用LV_MEM_POOL_ALLOC,意思是用自己函数的初始化内存.
...
#else  // 2 //给出地址
    tlsf = lv_tlsf_create_with_pool((void *)LV_MEM_ADR, LV_MEM_SIZE); // LV自带算法,给出地址
#endif // 2

#endif // 3

#if LV_MEM_ADD_JUNK // 4
