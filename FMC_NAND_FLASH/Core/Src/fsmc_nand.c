...
uint8_t NAND_ReadStatus(void)
{
    ...
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0x70; // 发送读状态命令
 ...
 * @param ptr
 */
void NAND_read_id(uint8_t *ptr)
{
    *(volatile uint8_t *)(NAND_BASE | cmd) = NAND_READ_ID;
    *(volatile uint8_t *)(NAND_BASE | addr) = 0;
    ptr[0] = *(volatile uint8_t *)(NAND_BASE | data);
    ptr[1] = *(volatile uint8_t *)(NAND_BASE | data);
    ptr[2] = *(volatile uint8_t *)(NAND_BASE | data);
    ptr[3] = *(volatile uint8_t *)(NAND_BASE | data);
    ptr[4] = *(volatile uint8_t *)(NAND_BASE | data);
}
/**
 * @brief 结构体初始化
 *
...
uint8_t NAND_page_read(uint32_t page_add, uint16_t byte_add, uint8_t *buffer, uint16_t num_byte_to_read)
{
    ...
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)byte_add;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(byte_add >> 8);
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)page_add;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(page_add >> 8);
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(page_add >> 16);
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0x30;
    ...
    if (num_byte_to_read % NAND_ECC_SECTOR_SIZE) // 不是NAND_ECC_SECTOR_SIZE的整数倍，不进行ECC校验
    {
        ...
        {
            buffer[i] = *(volatile uint8_t *)(NAND_BASE | data);
        }
    }
    else
    {
        ...
        *(volatile uint8_t *)(NAND_BASE | cmd) = 0X05;    // 随机读指令
        // 发送地址
        *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)i;
        *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(i >> 8);
        *(volatile uint8_t *)(NAND_BASE | cmd) = 0XE0; // 开始读数据
...
uint8_t NAND_Write_Page(uint32_t page_add, uint16_t byte_add, uint8_t *buffer, uint16_t num_byte_to_read)
{
    ...
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0x80;
    // 发送地址
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)byte_add;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(byte_add >> 8);
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)page_add;
    ...
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(page_add >> 16);
    ...
    else
    {
        ...
        NAND_Delay(30);                                   // 等待
        *(volatile uint8_t *)(NAND_BASE | cmd) = 0X85;    // 随机写指令
        // 发送地址
        *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)i;
        *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(i >> 8);
    ...
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0x10;
    if (NAND_WaitForReady() != NSTA_READY)
...
int8_t NAND_CopyPageWithoutWrite(uint32_t Source_PageNum, uint32_t Dest_PageNum)
{
    ...
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0; // 发送命令0X00
    // 发送源页地址
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)0;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)0;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)Source_PageNum;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(Source_PageNum >> 8);
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(Source_PageNum >> 16);
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0x35; // 发送命令0X35

    NAND_wait_busy();                              // 超时退出
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0x85; // 发送命令0X85
    // 发送目的页地址
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)0;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)0;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)Dest_PageNum;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(Dest_PageNum >> 8);
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(Dest_PageNum >> 16);
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0x10; // 发送命令0X10
...
uint8_t NAND_EraseBlock(uint32_t BlockNum)
{
    ...
    BlockNum <<= 6; // 块地址变页地址
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0x60;
    // 发送块地址
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)BlockNum;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(BlockNum >> 8);
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(BlockNum >> 16);
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0xd0;
    Delay_ms(4); // 等待擦除成功
    if (NAND_WaitForReady() != NSTA_READY)
        return NSTA_ERROR; // 失败
    return 0;              // 成功
...
uint8_t NAND_WritePageConst(uint32_t PageNum, uint16_t ColNum, uint32_t cval, uint16_t NumByteToWrite)
{
    ...
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0X80;
    // 发送地址
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)ColNum;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(ColNum >> 8);
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)PageNum;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(PageNum >> 8);
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(PageNum >> 16);
    NAND_Delay(30);                      // 等待tADL
...
{
    uint16_t i = 0;
    *(volatile uint8_t *)(NAND_BASE | cmd) = 0X00;
    // 发送地址
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)ColNum;
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)(ColNum >> 8);
    *(volatile uint8_t *)(NAND_BASE | addr) = (uint8_t)PageNum;
