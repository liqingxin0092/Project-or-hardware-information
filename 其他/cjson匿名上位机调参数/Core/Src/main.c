...
int main(void)
{
  ...
  MX_TIM1_Init();
  /* USER CODE BEGIN 2 */
  HAL_TIM_PWM_Start(&htim9,TIM_CHANNEL_1);//电机B
  HAL_TIM_PWM_Start(&htim9,TIM_CHANNEL_2);//电机B
  HAL_TIM_PWM_Start(&htim10,TIM_CHANNEL_1);//电机A
  HAL_TIM_PWM_Start(&htim11,TIM_CHANNEL_1);//电机A
  HAL_TIM_PWM_Start(&htim12,TIM_CHANNEL_2);//舵机
  HAL_TIM_Encoder_Start(&htim2,TIM_CHANNEL_ALL);//右轮速度
  HAL_TIM_Encoder_Start(&htim3,TIM_CHANNEL_ALL);//左轮速度
  HAL_TIM_Base_Start_IT(&htim1);//定时器中断
  __HAL_UART_ENABLE_IT(&huart3,UART_IT_RXNE);   //开启串口1接收中断
  PID_init();//初始化车速控制PID
...
//   ANO_DT_Send_F2(100*MotorASpeed, 100*MotorBSpeed, 200,-200);
      printf("%g,%g\r\n",pidMotorASpeed.target_val,MotorBSpeed);
//   HAL_Delay(10);
