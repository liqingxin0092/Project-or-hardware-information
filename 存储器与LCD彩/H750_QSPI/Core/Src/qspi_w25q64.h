...
int8_t QSPI_W25Qxx_Init(QSPI_HandleTypeDef *p);             // W25Qxx初始化
int8_t QSPI_W25Qxx_Reset(QSPI_HandleTypeDef *p);            // 复位器件
uint32_t QSPI_W25Qxx_ReadID(QSPI_HandleTypeDef *p);         // 读ID
int8_t QSPI_W25Qxx_MemoryMappedMode(QSPI_HandleTypeDef *p); // 进入内存映射模式
uint8_t QSPI_W25Qxx_Read_Status_reg1(QSPI_HandleTypeDef *p);
int8_t QSPI_W25Qxx_WriteEnable(QSPI_HandleTypeDef *p);

int8_t QSPI_W25Qxx_SectorErase(QSPI_HandleTypeDef *p, uint32_t SectorAddress);    // 扇区擦除，4K字节， 参考擦除时间 45ms
int8_t QSPI_W25Qxx_BlockErase_32K(QSPI_HandleTypeDef *p, uint32_t SectorAddress); // 块擦除，  32K字节，参考擦除时间 120ms
int8_t QSPI_W25Qxx_BlockErase_64K(QSPI_HandleTypeDef *p, uint32_t SectorAddress); // 块擦除，  64K字节，参考擦除时间 150ms，实际使用建议使用64K擦除，擦除的时间最快
int8_t QSPI_W25Qxx_ChipErase(QSPI_HandleTypeDef *p);                              // 整片擦除，参考擦除时间 20S

int8_t QSPI_W25Qxx_WritePage(QSPI_HandleTypeDef *p, uint8_t *pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite); // 按页写入，最大256字节
int8_t QSPI_W25Qxx_WriteBuffer(QSPI_HandleTypeDef *p, uint8_t *pData, uint32_t WriteAddr, uint32_t Size);           // 写入数据，最大不能超过flash芯片的大小
int8_t QSPI_W25Qxx_ReadBuffer(QSPI_HandleTypeDef *p, uint8_t *pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead);  // 读取数据，最大不能超过flash芯片的大小

int8_t QSPI_W25Qxx_Test(QSPI_HandleTypeDef *p); // Flash读写测试
