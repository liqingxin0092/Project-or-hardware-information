...
int8_t QSPI_W25Qxx_Init(QSPI_HandleTypeDef *p)
{
	...
	QSPI_W25Qxx_Reset(p);			   // 复位器件
	Device_ID = QSPI_W25Qxx_ReadID(p); // 读取器件ID
...
uint32_t QSPI_W25Qxx_ReadID(QSPI_HandleTypeDef *p)
{
	...
	if (HAL_QSPI_Command(p, &s_command, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
	{
		return (uint32_t)W25Qxx_ERROR_INIT; // 如果发送失败，返回错误信息
	}
	// 接收数据
	if (HAL_QSPI_Receive(p, QSPI_ReceiveBuff, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
	{
		return (uint32_t)W25Qxx_ERROR_TRANSMIT; // 如果接收失败，返回错误信息
	}
	// 将得到的数据组合成ID
...
int8_t QSPI_W25Qxx_MemoryMappedMode(QSPI_HandleTypeDef *p)
{
	...
	QSPI_W25Qxx_Reset(p); // 复位W25Qxx

	if (HAL_QSPI_MemoryMapped(p, &s_command, &s_mem_mapped_cfg) != HAL_OK) // 进行配置
	{
...
uint8_t QSPI_W25Qxx_Read_Status_reg1(QSPI_HandleTypeDef *p)
{
	...
	HAL_QSPI_Command(p, &s_command, HAL_QPSI_TIMEOUT_DEFAULT_VALUE); // 发送读取命令.
...
int8_t QSPI_W25Qxx_Test(QSPI_HandleTypeDef *p)	  // Flash读写测试
{
	...
	QSPI_Status = QSPI_W25Qxx_BlockErase_32K(p, W25Qxx_TestAddr); // 擦除32K字节
	...
	ExecutionTime_Begin = HAL_GetTick();																 // 获取 systick 当前时间，单位ms
	QSPI_Status = QSPI_W25Qxx_WriteBuffer(p, W25Qxx_WriteBuffer, W25Qxx_TestAddr, W25Qxx_NumByteToTest); // 写入数据
	ExecutionTime_End = HAL_GetTick();																	 // 获取 systick 当前时间，单位ms
	...
	ExecutionTime_Begin = HAL_GetTick();															   // 获取 systick 当前时间，单位ms
	QSPI_Status = QSPI_W25Qxx_ReadBuffer(p, W25Qxx_ReadBuffer, W25Qxx_TestAddr, W25Qxx_NumByteToTest); // 读取数据
	ExecutionTime_End = HAL_GetTick();																   // 获取 systick 当前时间，单位ms
