...
#define WIFI_SSID "vivopxd"					//WIFI用户名
#define WIFI_PASSWORD "12345678"				//WIFI密码
#define ONENET_MQTT_PRODUCT_ID "1vxF973ZQp"		//OneNET MQTT产品ID
#define ONENET_MQTT_DEVICE_NAME "esp8266"	//OneNET MQTT设备名称
#define ONENET_MQTT_TOKEN "version=2018-10-31&res=products%2F1vxF973ZQp%2Fdevices%2Fesp8266&et=2061393849&method=md5&sign=Y6Nhun9Hb3%2F0alE%2F6vU3Ug%3D%3D"//设备token		
...
/**   
  ...
  */
OneNET_MQTT_Data ChuangLian_Control = {"chuanglian_ctrl", TYPE_BOOL, {.bool_value = W_FALSE}};
OneNET_MQTT_Data JiaShiQi_Control = {"jiashiqi_ctrl", TYPE_BOOL, {.bool_value = W_FALSE}};
OneNET_MQTT_Data sensor_status = {"sensor_state", TYPE_BOOL, {.bool_value = W_FALSE}};
...
/**   
  ...
  */
#define wireless_serial_init    USART2_Config
...
{
      uart_transmit_string(USART2,str);
}
...
/**  
  ...
  * @返回值 0：不符合预期接收数据
			1：符合预期接收数据
  */
uint8_t wireless_send_command(const uint8_t *cmd, char *res, uint8_t sendCount, uint8_t clear_buffer, uint16_t delay_xms, uint8_t printf_enable)
{
...
void wireless_init(void)
{
	...
	//用于配置MQTT客户端的用户参数                                                                                                         
	if(wireless_send_command(ONENET_MQTT_USERCFG_INFO, "OK", 3, W_TRUE, 1000, W_ENABLE) != W_OK)		WirelessStatus.error_code |= (1 << 5);		
	//连接onenet的MQTT服务器                                                                                                          
	if(wireless_send_command(ONENET_MQTT_SERVER_INFO, "OK", 3, W_TRUE, 3000, W_ENABLE) != W_OK) 		WirelessStatus.error_code |= (1 << 6) | ERROR_MQTT_CONNECT;		
	//订阅设备属性上报响应主题	                                                                                                        
	if(wireless_send_command(ONENET_MQTT_REPLY_TOPIC, "OK", 3, W_TRUE, 2000, W_ENABLE) != W_OK) 		WirelessStatus.error_code |= (1 << 7);
#if ONENET_MQTT_SET_ENABLE	
	//订阅设备属性设置请求                                                                                                          
	if(wireless_send_command(ONENET_MQTT_SET_TOPIC, "OK", 3, W_TRUE, 1000, W_ENABLE) != W_OK) 		WirelessStatus.error_code |= (1 << 8);	
#endif	
	...
	else 		//如果有错误
	{
        ...
        printf("%d",WirelessStatus.error_code);
...
{ 
	ChuangLian_Control.value.bool_value = 0;       	//赋值给onenet对应的属性值，注意属性的数据类型
	JiaShiQi_Control.value.bool_value = 0;       		//赋值给onenet对应的属性值，注意属性的数据类型
	temperature.value.float_value +=0.1;       			//赋值给onenet对应的属性值，注意属性的数据类型
	humidity.value.float_value = 0;         			//赋值给onenet对应的属性值，注意属性的数据类型
}
...
void wireless_publish_data(void) 
{
	...
	wireless_send_command((const uint8_t *)atCommand, "OK", 1, W_TRUE, 25, W_ENABLE);    // 发送 AT 命令进入上报长数据模式
    // 发送数据
    if (wireless_send_command((const uint8_t *)globalBuffer, "{\"id\":\"123\",\"code\":200,\"msg\":\"success\"}", 1, W_TRUE, 300, W_ENABLE) != W_OK) error_send_count++;
...
void wireless_receive_command_respond(uint32_t respond_id, uint16_t code, char *msg)
{
	...
	wireless_send_command((const uint8_t *)jsonData, "OK", 1, W_TRUE, 50, W_ENABLE);
}
