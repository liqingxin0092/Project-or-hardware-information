...
//  AFIO->EXTICR[1]|=0<<4; /*这里默认选择portA的引脚进入EXTI*/
    EXTI->FTSR |= 1 << 5; // 选择线5上的下降沿触发
    EXTI->IMR |= 1 << 5;  // 开放线5上的中断请求
        
    NVIC_SetPriority(EXTI9_5_IRQn,NVIC_EncodePriority(NVIC_PriorityGroup_4, 6, 0));
