...
{
    volatile u32 cfg1_temp,cfg2_temp;
    /*RCC */
   ...
   __SPI_16BIT();
    ...
    while(fercount>0)
    {
        if(ACTIVE_SPI->SR&2)//TXP标志位,如果有空闲位置才进行
        {
            if(fercount>1)
            {
              *((__IO uint32_t *)&ACTIVE_SPI->TXDR) =  (uint32_t)(((uint32_t)LCD.BackColor<<16)|LCD.BackColor);
              fercount -= 2;               
            }
            else//只剩一个数据
            {
...
void LCD_Init(void)
{
    __IO __ALIGNED(4) uint8_t temp[14];
    gpio_init_pin(GPIO_E,12,2,0,3,0,5);//clk
    gpio_init_pin(GPIO_E,14,2,0,3,0,5);//mosi
    gpio_init_pin(GPIO_D,15,1,0,0,0,0);//blk
    gpio_init_pin(GPIO_E,15,1,0,3,0,0);//rs
    gpio_init_pin(GPIO_E,11,2,0,3,0,5);//cs
    lcd_spi_init(); // spi初始化
