...
{	
	u8	res;//返回值
	u8 temp;	
    LCD_FillRect( x,  y,  width,  height,LCD.BackColor);
...
void picture_scan_and_display(uint16_t x,uint16_t y,uint16_t width,uint16_t height,uint16_t display_time)
{
        ...
        /*为图片信息申请空间*/
        FILINFO file_temp;
        picfileinfo = &file_temp; // 给文件信息申请内存
        uint8_t a[41];
        uint8_t b[40];
        pname = (void*)a;               // 为带路径的文件名分配内存
        picoffsettbl = (void*)b; // 申请4*totpicnum个字节的内存,用于存放图片索引
