...
__ALIGNED(4) u8 SDMMC_DATA_BUFFER[512];						  
...
SD_Error SD_Init(void)
{
    ...
    gpio_init_pin(GPIO_C,8,2,0,3,0,12);
    gpio_init_pin(GPIO_C,9,2,0,3,0,12);
    gpio_init_pin(GPIO_C,10,2,0,3,0,12);
    gpio_init_pin(GPIO_C,11,2,0,3,0,12);
    gpio_init_pin(GPIO_C,12,2,0,3,0,12);
    gpio_init_pin(GPIO_D,2,2,0,3,0,12);
    ...
    __set_priority(SDMMC1_IRQn,2,0);  //配置SDMMC1中断，抢占优先级2，子优先级0
    __enable_IRQ(SDMMC1_IRQn);        //使能SDMMC1中断
...
SD_Error SD_ReadBlocks(u8 *buf,long long addr,u16 blksize,u32 nblks)
{
	...
	DISNABLE_ALL_CONFIGURABLE_INTERRUPTS();//关闭总中断(POLLING模式,严禁中断打断SDMMC读写操作!!!)
	...
	ENABLE_ALL_CONFIGURABLE_INTERRUPTS();				//开启总中断
