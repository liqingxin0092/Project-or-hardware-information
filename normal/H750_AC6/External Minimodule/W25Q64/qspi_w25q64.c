...
void HAL_QSPI_MspInit(QSPI_HandleTypeDef *hqspi)
{
	...
	if (hqspi->Instance == QUADSPI)
	{
		...
		gpio_init_pin(GPIO_B, 2, 2, 0, 3, 0, 9);  // 时钟线
		gpio_init_pin(GPIO_B, 6, 2, 0, 3, 0, 10); // NSS
		gpio_init_pin(GPIO_D, 11, 2, 0, 3, 0, 9); // IO0
		gpio_init_pin(GPIO_D, 12, 2, 0, 3, 0, 9); // IO1
		gpio_init_pin(GPIO_E, 2, 2, 0, 3, 0, 9);  // IO2
		gpio_init_pin(GPIO_D, 13, 2, 0, 3, 0, 9); // IO3
	}
}
...
// 实验平台：鹿小班STM32H750VBT6核心板 （型号：LXB750M1-VBT6）
