...
void FTL_BadBlockMark(uint32_t blocknum)
{
    ...
    NAND_Write_Page(blocknum * nand_dev.block_pagenum, 0, (uint8_t *)&temp, 4); // 在第一个page的spare区,第一个字节做坏块标记(前4个字节都写)
    NAND_Write_Page(blocknum * nand_dev.block_pagenum + 1, 0, (uint8_t *)&temp, 4);
...
{
    uint8_t flag = 0;
    NAND_page_read(blocknum * nand_dev.block_pagenum, 2048, &flag, 1); // 读取坏块标志
    if (flag == 0XFF)                                                  // 好块?,读取备份区坏块标记
    {
        NAND_page_read(blocknum * nand_dev.block_pagenum + 1, 2048, &flag, 1); // 读取备份区坏块标志
        if (flag == 0XFF)
            return 0; // 好块
        else
            return 1; // 坏块
...
uint8_t FTL_UsedBlockMark(uint32_t blocknum)
{
    ...
    uint8_t temp = 0;
    temp = NAND_Write_Page(blocknum * nand_dev.block_pagenum, 2049, (uint8_t *)&Usedflag, 1);
    return temp;
}
...
uint32_t FTL_FindUnusedBlock(uint32_t sblock, uint8_t flag)
{
    ...
    for (blocknum = sblock + 1; blocknum > 0; blocknum--)
    {
        ...
        {
            NAND_page_read((blocknum - 1) * nand_dev.block_pagenum, 2048, (uint8_t *)&temp, 4);
            if (temp == 0XFFFFFFFF)
...
uint8_t FTL_WriteSectors(uint8_t *pBuffer, uint32_t SectorNo, uint16_t SectorSize, uint32_t SectorCount)
{
    ...
    for (i = 0; i < SectorCount; i++)
    {
        ...
        if (flag)
            return 2; // 读写错误，坏块
        if (temp == (wlen / 4))
            flag = NAND_Write_Page(PhyPageNo, PageOffset, pBuffer, wlen); // 全为0XFF,可以直接写数据
        else
...
uint8_t FTL_ReadSectors(uint8_t *pBuffer, uint32_t SectorNo, uint16_t SectorSize, uint32_t SectorCount)
{
    ...
    for (i = 0; i < SectorCount; i++)
    {
        ...
        flag = NAND_page_read(PhyPageNo, PageOffset, pBuffer, rsecs * SectorSize); // 读取数据
        if (flag == NSTA_ECC1BITERR)                                               // 对于1bit ecc错误,可能为坏块
        {
            flag = NAND_page_read(PhyPageNo, PageOffset, pBuffer, rsecs * SectorSize); // 重读数据,再次确认
...
uint8_t FTL_CreateLUT(uint8_t mode)
{
    ...
    for (i = 0; i < nand_dev.block_totalnum; i++)
    {
        ...
        NAND_page_read(i * nand_dev.block_pagenum, nand_dev.page_mainsize, buf, 4);

        // printf("buf[0]=%d\r\n", buf[0]);

        if (buf[0] == 0XFF && mode)
            NAND_page_read(i * nand_dev.block_pagenum + 1, nand_dev.page_mainsize, buf, 1); // 好块,且需要检查2次坏块标记
        if (buf[0] == 0XFF)
        {
...
uint8_t FTL_Format(void)
{
    ...
    for (i = 0; i < nand_dev.block_totalnum; i++) // 在好块中标记上逻辑块信息
    {
        ...
        if (temp == 0)               // 好块
        {
            NAND_Write_Page(i * nand_dev.block_pagenum, 2050, (uint8_t *)&n, 2); // 写入逻辑块编号
            n++;                                                                 // 逻辑块编号加1
            if (n == goodblock)
                break; // 全部标记完了
        }
