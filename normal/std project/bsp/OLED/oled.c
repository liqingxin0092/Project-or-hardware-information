...
//显示小数点后三位
void oled_show_float(u8 x,u8 y,float num,u8 len,u8 size1,u8 mode)
{
     uint32_t num_x1000=num*1000;
     uint8_t temp,t,i,m=0;
     if(size1==8)m=2;
	 for(t=0;t<len;t++)
  	 {
		temp=(((uint32_t)num)/OLED_Pow(10,len-t-1))%10;
        if(temp==0)
        {
            OLED_ShowChar(x+(size1/2+m)*t,y,'0',size1,mode);
        }
     ...
     for(i=0;i<3;i++)
     {
        temp=((num_x1000%1000)/OLED_Pow(10,2-i))%10;
        if(temp==0)
        {
            OLED_ShowChar(x+(size1/2+m)*(i+t+1),y,'0',size1,mode);
        }
        else 
        {
            OLED_ShowChar(x+(size1/2+m)*(i+t+1),y,temp+'0',size1,mode);
        }
     }
}
