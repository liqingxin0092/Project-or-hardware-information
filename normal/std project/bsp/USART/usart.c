...
void BSP_USART_IRQHandler(void)
{
    ...
    if (usart_interrupt_flag_get(BSP_USART, USART_INT_FLAG_IDLE) == SET) // 检测到帧中断
    {
        ...
        /* 处理DMA接收到的数据 */
        g_recv_length = USART_RECEIVE_LENGTH - dma_transfer_number_get(DMA1, BSP_DMA_CH); // 计算实际接收的数据长度
        /* 重新设置DMA传输 */
        dma_channel_disable(DMA1, BSP_DMA_CH); 		  // 失能DMA通道
        usart_dma_config();                           // 重新配置DMA进行传输
#endif
...
void usart_dma_config(void)
{
	...
	/* 使能DMA中断 */
	dma_interrupt_enable(DMA1,BSP_DMA_CH,DMA_CHXCTL_FTFIE);  // 使能传输完成中断
